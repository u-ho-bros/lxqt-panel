...
void LXQtWorldClock::settingsChanged()
{
    ...
    QList<QHash<QString, QVariant> > array = _settings->readArray(QLatin1String("timeZones"));
    for (const auto &hash : array)
    {
        QString timeZoneName = hash.value(QLatin1String("timeZone"), QString()).toString();
        mTimeZones.append(timeZoneName);
        mTimeZoneCustomNames[timeZoneName] = hash.value(QLatin1String("customName"),
                                                        QString()).toString();
    }
